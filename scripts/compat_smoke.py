...
launcher=ROOT/'cli/bin/lazydev.cjs'
if not launcher.is_file(): errors.append('npm launcher missing at cli/bin/lazydev.cjs')
else:
    text=launcher.read_text(encoding='utf-8')
    if '../../scripts/lazydev.mjs' not in text: errors.append('npm launcher must resolve the runtime from the repository root')
pkg=load('package.json')
if pkg and pkg.get('bin',{}).get('lazydev')!='./cli/bin/lazydev.cjs': errors.append('package.json lazydev bin target mismatch')
